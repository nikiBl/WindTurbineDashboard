...
exports.updateFailures = async (req, res) => {
    ...
    try {
        ...
        const timeOfFailure = req.query.etime    //new Date(req.query.itime)
        const desc = req.query.desc

        const resTemp = await pgPool.query(`UPDATE FAILURES SET END_TIME_OF_FAILURE = to_timestamp('${timeOfFailure}', 'DD/MM/YYYYHH24:MI:SS'), FAILURE_DESC='${desc}' WHERE ID = (SELECT MAX(ID) FROM FAILURES WHERE ID_TURBINE=${turbineId})`, (err, result) => {
            if(err) {
